...
export function NyxChatScreen({ userId, profile, onBack }: { userId: string; profile: UserProfile; onBack: () => void }) {
  ...
  return <div
    ...
    {/* Input */}
    <div style={{ position: "relative", zIndex: 10, padding: "10px 14px 16px", background: C.nav, backdropFilter: "blur(24px) saturate(145%)", borderTop: `1px solid ${isSim ? "rgba(255,154,60,0.15)" : C.border}` }}>
      ...
      <div ref={inputRef} style={{ position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8, background: C.bgCard, border: `1px solid ${showEmoji ? `${ac}55` : C.border}`, borderRadius: 26, padding: "8px 8px 8px 14px", transition: "border-color .2s", boxShadow:"0 5px 18px rgba(57,42,101,0.06)" }} onFocusCapture={e => (e.currentTarget.style.borderColor = `${ac}55`)} onBlurCapture={e => { if (!showEmoji) e.currentTarget.style.borderColor = C.border; }}>

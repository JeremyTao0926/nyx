  ...
  return (
    <div style={{ fontSize:14.5, lineHeight:1.8, color:C.text, wordBreak:"break-word" }}>
      {text.split("\n").map((line,i) => {
        ...
        if (/^缺點/.test(s)) return <div key={i} style={{ color:C.warning, marginTop:3 }}>{s}</div>;
...
export function Lightbox({ lb, onClose }:{ lb:LB; onClose:()=>void }) {
  const [idx,setIdx]=useState(lb?.index ?? 0);
  useEffect(()=>{
    if (!lb) return;
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,zIndex:300,background:C.overlayStrong,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",animation:"fadeIn .15s ease" }}>
...
const PARTICLE_OFFSETS = [
  [-112,-74],[-76,-104],[-35,-88],[18,-112],[68,-82],[110,-48],
  [-122,18],[-82,62],[-28,88],[30,72],[78,52],[118,12],
];
...
export function MatchAnimation({ myAvatar,myName,theirAvatar,theirName,onChat,onIcebreaker,onContinue }:
  ...
  const particles=PARTICLE_OFFSETS.map(([x,y],i)=>({id:i,e:["♥","✦","★","◆","❋","✿"][i%6],x,y}));
  return (
    <div style={{ position:"fixed",inset:0,zIndex:500,background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",animation:"fadeIn .3s ease" }}>
      ...
      {ph>=2&&<div style={{ position:"absolute",top:"44%",left:"50%",zIndex:0 }}>
        {particles.map(h=><div key={h.id} style={{ position:"absolute",fontSize:14,opacity:0,color:h.id%2===0?C.gold:C.rose,animation:`heartBurst .85s ${h.id*.055}s ease-out forwards`,"--x":`${h.x}px`,"--y":`${h.y}px`} as CSSProperties & Record<"--x"|"--y", string>}>{h.e}</div>)}
      </div>}
      {/* Text */}
...
export function NyxAnalysisSheet({ msg,isUser,ctx,gender,mbti,cache,onCache,onClose }:
  { msg:Msg;isUser:boolean;ctx:Msg[];gender:"male"|"female";mbti:string;cache:Map<string,string>;onCache:(id:string,r:string)=>void;onClose:()=>void }) {
  const [result,setResult]=useState(cache.get(msg.id)||"");
  const [loading,setLoading]=useState(!cache.has(msg.id));
  const started=useRef(false);
  useEffect(()=>{
    if(started.current||cache.has(msg.id))return;
    started.current=true;
    async function analyze(){
      const ctxStr=ctx.slice(-6).map(m=>`${m.from==="user"?"我":"對方"}：${m.text||"[圖片]"}`).join("\n");
    ...
    void analyze();
  },[cache,ctx,gender,isUser,mbti,msg,onCache]);
...
export function CherryBlossoms() {
  const petals=[
    {id:0,left:14,delay:0,dur:12,size:5},{id:1,left:28,delay:4,dur:14,size:7},
    {id:2,left:42,delay:9,dur:11,size:4},{id:3,left:58,delay:2,dur:15,size:6},
    {id:4,left:72,delay:7,dur:13,size:5},{id:5,left:88,delay:11,dur:16,size:7},
  ];
  return (
